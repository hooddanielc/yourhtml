...
static std::function<void()> before_each_fn = []() {};
static std::function<void()> after_each_fn = []() {};

inline void BEFORE_EACH(std::function<void()> lambda) {
  before_each_fn = lambda;
}

inline void AFTER_EACH(std::function<void()> lambda) {
  after_each_fn = lambda;
}
